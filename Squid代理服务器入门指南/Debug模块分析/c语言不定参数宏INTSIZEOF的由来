...
## 二、理解
这个宏的整体结构就是输入的字节数是4个字节的多少个，然后在乘以4字节就是宏的值，
比如n是double，那么sizeof(double)就是8字节，也就是sizeof(int)的2倍，然后再乘以4字节（也就是2*4 = 8）字节那么这个宏就是8字节，
若是char，那么这个宏的值就是4字节。
...
当然"/"就如c语言中的“/”一个意思是商取整数的形式。这里简单的列出证明的过程：
